...
class TestIngredients(TestCase):
    ...
    def test_remove_list_ingredient(self):
        ...
        create_user_list_ingredients('test_user2', 'test_listname', None)
        remove_list_ingredient('test_user2', 'test_listname', ing2.get('ingredient_name'), ing2.get('unit'))
        list = get_user_lists_ingredients(test_user2.username, test_user2.id)
        self.assertEqual(list[0].ingredients, None)

        create_user_list_ingredients(test_user.username, 'test_listname', ing_list)
        list = get_user_lists_ingredients(test_user.username, test_user.id)
        remove_list_ingredient('test_user', 'test_listname', ing2.get('ingredient_name'), ing2.get('unit'))
...
class AddIngredientToListApi(TestCase):
    ...
    def test_add_ingredient_to_list_missing_data(self, mock_decode):
        ...
        self.assertDictEqual(
            response.json(),
            {
                'message': UserListIngredientsAPIView.MISSING_ADD_INGREDIENT_MSG
            }
        )
...
class CreateUser(TestCase):
    ...
    def test_create_user_api_without_email_token_returns_500(self, mock_decode):
        ...
        self.assertDictEqual(
            response.json(),
            {
                'message': UserAPIView.MISSING_USER_INFO
            }
        )
